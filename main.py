...
def plot_data(data, indicators, sync_axis=None):
    ...
    if sync_axis is not None:
        p = figure(x_axis_type="datetime", tools="pan, wheel_zoom,box_zoom, reset,save", width = 1000,
                   x_range=sync_axis, x_axis_label= 'Date', y_axis_label = "Price in Dollars")
    else: 
        p = figure(x_axis_type="datetime", tools="pan, wheel_zoom,box_zoom, reset,save", width = 1000,
                   x_axis_label= 'Date', y_axis_label = "Price in Dollars")
...
load_button = Button(label='Load Date', button_type="success")
